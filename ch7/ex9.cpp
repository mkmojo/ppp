...
{
    symbol_table.set(s, d);
}

void Symbol_table::define(Variable v)
{
    symbol_table.define(v);
}

bool Symbol_table::is_declared(string s)
{
    return symbol_table.is_declared(s);
}

bool Symbol_table::isconst(string s)
{
    return symbol_table.isconst(s);
}
        ...
        case name:
            {
                ...
                if( t2.kind != '='){
                    ts.unget(t2);
                    return st.get_value(name);
                }
                double d = expression();
                if ( st.isconst(name) ) error(name + " is const var");
                st.set_value(name, d);
                if(d != st.get_value(name)) error("fail to set value for ", name);
                return d;
            }
        default:
...
double declaration()
{
    ...
    if (t.kind == cst){
        ...
        if (!st.is_declared(var.name)) error(t.name, " not declared , cannot reset value");
        Token t2 = ts.get();
        if (t2.kind != '=') error("= missing in declaration of ", t.name);
        double d = expression();
        st.define(Variable(t.name,d, true)); //store new Variable in the global table
        return d;
    }else{
        Token t = ts.get();
        if (t.kind != name) error ("name expected in declaration");
        if (st.is_declared(t.name)) error(t.name, " declared twice");
        Token t2 = ts.get();
        if (t2.kind != '=') error("= missing in declaration of ", t.name);
        double d = expression();
        st.define(Variable(t.name,d)); //store new Variable in the global table
        return d;
    }
}
...
double assignment()
{
    ...
    double d = expression();
    if( !st.is_declared(t.name) ) // if does not declared, create one with init value 0
        st.define(Variable(t.name, 0, true)); 
