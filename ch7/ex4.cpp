...
class Symbol_table {
    public:
        ...
        double is_declared(string s);
        double isconst(string s);
};
...
{
    for (int i = 0; i<st.names.size(); ++i)
        if (st.names[i].name == s && st.names[i].isconst) return true;
    return false;
}
...
double declaration()
{
    ...
    if (t.kind == cst){
        ...
        double d = expression();
        st.names.push_back(Variable(t.name,d, true)); //store new Variable in the global table
        return d;
    }else{
        Token t = ts.get();
        ...
        if (t2.kind != '=') error("= missing in declaration of ", t.name);
        double d = expression();
        st.names.push_back(Variable(t.name,d)); //store new Variable in the global table
        return d;
    }
}
...
double assignment()
{
    ...
    if( !is_declared(t.name) ) // if does not declared, create one with init value 0
        st.names.push_back(Variable(t.name, 0, true)); 
    set_value(t.name, d);
    return d;
}
